 ...
 * @author Ivan Voskoboynyk <http://github.com/e1himself>
 */
class CurlyRouteCompiler implements RouteCompilerInterface
{
  ...
  private static function compilePattern(Route $route, $pattern, $isHost)
  {
    ...
    preg_match_all('#'.self::VARIABLE_NAME_PATTERN.'#', $pattern, $matches, PREG_OFFSET_CAPTURE | PREG_SET_ORDER);
    foreach ($matches as $match) {
      ...
      if (null === $regexp) {
        ...
        if (('' !== $nextSeparator && !preg_match('#^'.self::VARIABLE_NAME_PATTERN.'#', $followingPattern)) || '' === $followingPattern) {
  ...
  {
    if ('' == $pattern) {
      // return empty string if pattern is empty or_ false (false which can be returned by substr)
      return '';
    }
    // first remove all placeholders from the pattern so we can find the next real static character
        $pattern = preg_replace('#'.self::VARIABLE_NAME_PATTERN.'#', '', $pattern);
